...
void changeLEDColor(int& currColorValue,int& newColorValue,int& thirdColorValue,uint8_t currPinColor,uint8_t newPinColor, const int delayTime){
  for(int i{0};i<255;++i){
    --currColorValue;
    ++newColorValue;
...
void loop(){
  ...
  changeLEDColor(::redValue,::greenValue,::blueValue,static_cast<uint8_t>(pinColor::RED),static_cast<uint8_t>(pinColor::GREEN),delayTime);

  //Change LED from green to blue.
  changeLEDColor(::greenValue,::blueValue,::redValue,static_cast<uint8_t>(pinColor::GREEN),static_cast<uint8_t>(pinColor::BLUE),delayTime);

  //Change LED from blue to red.
  changeLEDColor(::blueValue,::redValue,::greenValue,static_cast<uint8_t>(pinColor::BLUE),static_cast<uint8_t>(pinColor::RED),delayTime);
}
